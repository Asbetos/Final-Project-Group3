...
`core/data.py` provides the evaluation prompt pipeline used by both standard speculative decoding and EAGLE evaluation.
...
Training data for EAGLE is not handled here. That logic lives in `core/eagle3_train.py`.
...
2. EAGLE training examples come from Alpaca GPT-4 in `core/eagle3_train.py`.
